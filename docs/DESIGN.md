...
> Design notes for **property-creator-image-watermarker** — a runnable typescript example that Typed Node service that watermarks property creator images before publishing.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.image.process(...)`).
- **Feature code** — the actual task: property image publishing.
- **Configuration** — the API key is read from the `INFRAI_API_KEY` environment variable; no secret is ever hard-coded.

## Capabilities used

- `image.process` — mapped to `POST /v1/image/process`.
